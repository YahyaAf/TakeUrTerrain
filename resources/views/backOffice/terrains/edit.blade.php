...
<div class="p-6 bg-gradient-to-br from-blue-50 to-blue-50 min-h-screen">
    <div class="max-w-7xl mx-auto">
        <div class="relative mb-8">
            <h1 class="text-3xl font-bold text-blue-800 inline-block">Modifier un Terrain</h1>
        ...
        <div class="bg-white shadow-lg rounded-xl overflow-hidden border border-teal-50">
            ...
            <div class="p-6">
                <form action="{{ route('terrains.update', $terrain->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="text" id="name" name="name" value="{{ old('name', $terrain->name) }}" 
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="number" id="prix" name="prix" value="{{ old('prix', $terrain->prix) }}" 
                                    class="w-full pl-10 pr-4 py-3 border border-blue-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all"
                    ...
                    <div>
                        ...
                        <textarea id="description" name="description" rows="4"
                            ...
                            placeholder="Décrivez le terrain...">{{ old('description', $terrain->description) }}</textarea>
                        @error('description')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <div class="relative">
                                <div id="photoPreviewContainer" class="{{ $terrain->photo ? 'block' : 'hidden' }} mb-3 h-48 w-full rounded-lg border-2 border-dashed border-blue-300 flex items-center justify-center overflow-hidden">
                                    <img id="photoPreview" src="{{ $terrain->photo ? asset('storage/' . $terrain->photo) : '#' }}" 
                                         alt="Photo preview" 
                                         class="h-full w-full object-cover {{ $terrain->photo ? '' : 'hidden' }}">
                                </div>
                        ...
                        <div class="space-y-6">
                                ...
                                <select id="categorie_id" name="categorie_id" 
                                    ...
                                    @foreach($categories as $category)
                                        <option value="{{ $category->id }}" 
                                            {{ old('categorie_id', $terrain->categorie_id) == $category->id ? 'selected' : '' }}>
                                            {{ $category->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div>
                                ...
                                <select id="disponibility" name="disponibility" 
                                    ...
                                    <option value="disponible" {{ old('disponibility', $terrain->disponibility) == 'disponible' ? 'selected' : '' }}>Disponible</option>
                                    <option value="indisponible" {{ old('disponibility', $terrain->disponibility) == 'indisponible' ? 'selected' : '' }}>Indisponible</option>
                                </select>
                                @error('disponibility')
                                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                                @enderror
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="text" id="adresse" name="adresse" value="{{ old('adresse', $terrain->adresse) }}" 
                            ...
                            <select id="sponsors" name="sponsors[]" multiple
                                ...
                                @foreach($sponsors as $sponsor)
                                    <option value="{{ $sponsor->id }}" 
                                        {{ $terrain->sponsors->contains($sponsor->id) ? 'selected' : '' }}>
                                        {{ $sponsor->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div>
                        ...
                        <select id="tags" name="tags[]" multiple
                            ...
                            @foreach($tags as $tag)
                                <option value="{{ $tag->id }}" 
                                    {{ $terrain->tags->contains($tag->id) ? 'selected' : '' }}>
                                    {{ $tag->name }}
                                </option>
                            @endforeach
                        </select>
