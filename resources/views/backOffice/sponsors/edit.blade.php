...
<div class="p-6 bg-gradient-to-br from-blue-50 to-indigo-50 min-h-screen">
    <div class="max-w-7xl mx-auto">
        <div class="relative mb-8">
            <h1 class="text-3xl font-bold text-indigo-800 inline-block">Modifier un Sponsor</h1>
        ...
        <div class="bg-white shadow-lg rounded-xl overflow-hidden border border-indigo-50">
            ...
            <div class="p-6">
                <form action="{{ route('sponsors.update', $sponsor->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                    @csrf
                    @method('PUT')
                            ...
                            <div class="relative">
                                @if ($sponsor->logo)
                                    <div class="mb-4 flex items-center">
                                        <div class="h-20 w-20 rounded-full border-2 border-indigo-200 p-0.5 shadow-md overflow-hidden mr-4">
                                            <img src="{{ asset('storage/' . $sponsor->logo) }}" alt="{{ $sponsor->name }}" class="h-full w-full object-cover rounded-full">
                                        </div>
                                        <div class="text-sm text-indigo-600">
                                            <p class="font-semibold">Logo actuel</p>
                                            <p class="text-indigo-400 text-xs">Téléchargez un nouveau logo pour remplacer</p>
                                        </div>
                                    </div>
                                @endif
                                
                                <div id="logoPreviewContainer" class="hidden mb-3 h-24 w-24 rounded-full border-2 border-dashed border-indigo-300 flex items-center justify-center overflow-hidden">
                                ...
                                <div class="flex items-center justify-center border-2 border-dashed border-indigo-300 rounded-lg p-6">
                                    <div class="space-y-2 text-center">
                                        ...
                                        <p class="text-xs text-indigo-500 font-medium">Laissez vide pour conserver l'ancien logo</p>
                                    </div>
                                </div>
                                @error('logo')
                                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    ...
                    <div class="flex justify-end space-x-3 pt-6 border-t border-indigo-50">
                        ...
                        <button type="submit" 
                            ...
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" viewBox="0 0 20 20" fill="currentColor">
                                <path fill-rule="evenodd" d="M10 5a1 1 0 011 1v3h3a1 1 0 110 2h-3v3a1 1 0 11-2 0v-3H6a1 1 0 110-2h3V6a1 1 0 011-1z" clip-rule="evenodd" transform="rotate(45 10 10)" />
                            </svg>
